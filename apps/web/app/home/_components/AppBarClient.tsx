...
export function AppBarClient({ session }: { session: any }) {
  const router = useRouter();

  return (
    <header className="w-full px-6 py-4 shadow-md bg-white flex items-center justify-between">
      <h1
        className="font-bold tracking-wide hover:cursor-pointer"
          ...
          <div className="flex items-center gap-3">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="start"
                side="left"
                className="w-52 bg-white shadow-lg rounded-lg p-2 border border-gray-200"
              >
                <DropdownMenuItem
                  className="cursor-pointer flex items-center gap-2 px-3 py-2 hover:bg-gray-100 rounded-md text-gray-700"
                  onClick={() =>
                    session.user?.image &&
                    window.open(session.user.image, "_blank")
                  }
                >
